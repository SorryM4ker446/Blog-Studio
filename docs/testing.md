...
Browser denial is simulated at the JavaScript storage boundary; it does not claim coverage of every browser privacy policy or enterprise setting. These tests use the Chromium project and standalone deployment build. See [appearance preferences](preferences.md) for retention and compatibility rules.
...
Advanced-search category regressions verify that only Posts scope exposes an interactive label and dropdown, including server entry with a stale category in all scope. The category field remains mounted for CSS entry/exit transitions; inactive fields are inert, disabled and aria-hidden immediately, then display:none after exit. Browser tests inspect entry keyframes, partial exit opacity, reversing an unfinished exit, reduced-motion fallback, category clearing when leaving Posts, and history restoration of the previously selected category. Desktop and narrow-screen tests verify that switching an idle search scope never animates the unchanged keyword prompt; real result transitions and error recovery retain their existing coverage. Browsers without discrete display transitions fall back to immediate hiding. Query helpers additionally assert that hidden categories cannot constrain all/files searches.
...
These are local native-server Chromium checks, including API and PostgreSQL behavior through the existing test servers. They do not imply remote CI/container execution, Firefox/WebKit behavior, physical mobile-device testing or a production load benchmark.
...
CI discovers Vitest, Playwright and Go tests through its configured commands. CI runs for pull requests, manual dispatch and pushes to main/master, not an ordinary push to codex. Local native checks do not establish that GitHub Actions or the Linux container job has passed. The deployment still requires the Links migration; the browser recovery fixes require only rebuilding the frontend, with no recovery storage format change.
...
Login proxy regressions exercise the real router and isolated PostgreSQL database with IPv4/IPv6 trusted peers, a trusted forwarding chain, independent client budgets, cross-username blocking, spoofed headers from untrusted callers, missing/malformed forwarding data and successful-login reset. Links manager unit tests cover aborted reads, failed-read retry, write-failure invalidation, new drafts during pending writes, collection limits and account remounts. Proxy headers are simulated at the HTTP boundary locally; this is not production Caddy validation.

Links card layout regressions compare card dimensions and description/destination/action offsets for empty, short, maximum-length Latin/Chinese and unbroken text, including long URLs. Dark/light Chromium cases cover 1920px desktop and 375px mobile layouts, homepage cards and live-preview height stability. Measurements wait for dialog entrance animations to finish.

Link visibility control regressions cover dark/light themes at 1280px and 375px widths. They verify that the label ends beside its text, clicking the remaining row whitespace does not toggle visibility, and clicking the text or checkbox and pressing Space still work. Pointer interaction suppresses the outline even when Chromium retains `:focus-visible` after clicking an already keyboard-focused checkbox. Keyboard input or blur clears the local pointer flag, restoring the existing keyboard focus styling without forcing blur. Tests cover mouse-to-keyboard and keyboard-to-mouse transitions, Space toggling and Tab re-entry. Both outline and box-shadow are checked: the global input focus shadow applies only to text-entry controls, while checkbox and range keyboard focus retain their 2px outlines. Title, URL, description and untyped HEX fields retain the dark/light input shadows; all three color sliders remain shadow-free and respond to keyboard changes. The global allowlist includes untyped, text, search, password, email, URL, telephone and number inputs plus textareas, using zero-specificity type filtering to preserve existing component overrides. Browser geometry checks enforce vertical alignment between the checkbox and text.
...
CD trigger regressions verify that its source-branch allowlist excludes development branches and matches CI's supported push branches. They retain successful production push/manual paths for both main and master, and verify that a matching branch does not bypass PR rejection. Existing trust-gate tests cover unsuccessful CI, foreign repositories, unexpected workflows and the configured deployment branch. These are local configuration and expression checks; GitHub event delivery requires remote validation after the workflow reaches the default branch.
