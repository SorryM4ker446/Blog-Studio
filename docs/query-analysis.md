...
The September 15, 2026 native run on Go 1.26.8 and PostgreSQL 18.3 passed both opt-in suites: 45 historical baseline queries with six experiments, and 43 actual production queries with three index write comparisons. Each SELECT retains three full plans. The fixture remains 2,400 articles, 1,200 files, 40 categories and 138,112,234 Markdown bytes; disposable schemas roll back on completion.

Public rare-body search used `idx_posts_search_text` in all three samples (10.113 ms median); the common-title body index branch executed zero times in every sample (3.084 ms). Public mixed rare search measured 9.280 ms, administrator rare search 9.516 ms, and the hidden-term miss 1.448 ms. One-character Chinese still required a scan (670.915 ms); the dependency update does not remove this known cost. Administrator first-page and public-file first-page medians were 0.015 and 0.011 ms.

The actual body-index write comparison remained 117.952 ms without / 1,381.205 ms with the index for the rollback-only 100-row probe, at 3,686,400 index bytes. This supports the existing read/write tradeoff, not a new index policy. The optional alternative-GIN-queue experiment was not rerun because no query, index or migration changed. Historical SQL experimentation and production checks ran sequentially, after HTTP benchmarks and before E2E. These local plans do not establish the result on Linux CI or a deployment host.
